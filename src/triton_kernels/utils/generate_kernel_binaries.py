...
def compile_variant(kernel, constexprs):
    ...
    for i, name in enumerate(kernel.arg_names):
        if i in kernel.constexprs:
            continue

        if kernel.fn.__name__ == "flash_attention_kernel":
            # Explicit types for flash_attention_kernel
            if name in ("q_ptr", "k_ptr", "v_ptr", "o_ptr"):
                signature[name] = "*fp32"
            elif name == "sm_scale":
                signature[name] = "fp32"  # IMPORTANT: sm_scale is a float!
            else:
                signature[name] = "i32"
        elif kernel.fn.__name__ == "rotary_kernel":
            # Explicit types for rotary_kernel
            if name in ("OUT", "X", "COS", "SIN"):
                # data tensors
                signature[name] = "*fp32"
            elif name in (
                "CU_SEQLENS",       # unused when IS_VARLEN == False
                "SEQLEN_OFFSETS",   # scalar offset when IS_SEQLEN_OFFSETS_TENSOR == False
                "seqlen",
                "rotary_dim",
                "seqlen_ro",
                "stride_out_batch",
            ...
            # Original heuristic for other kernels
            if "ptr" in name:
                signature[name] = "*fp32"
            else:
                signature[name] = "i32"
